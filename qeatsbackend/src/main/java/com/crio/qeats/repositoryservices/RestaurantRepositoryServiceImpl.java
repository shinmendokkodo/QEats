...
public class RestaurantRepositoryServiceImpl implements RestaurantRepositoryService {
  ...
  /*
   * // TODO: CRIO_TASK_MODULE_NOSQL // Objective: // 1. Check if a restaurant is nearby and open.
   * If so, it is a candidate to be returned. // NOTE: How far exactly is "nearby"? public
   * List<Restaurant> findAllRestaurantsCloseBy(Double latitude, Double longitude, LocalTime
   * currentTime, Double servingRadiusInKms) {
   * 
   * List<Restaurant> restaurants = null; // TODO: CRIO_TASK_MODULE_REDIS // We want to use cache to
   * speed things up. Write methods that perform the same functionality, // but using the cache if
   * it is present and reachable. // Remember, you must ensure that if cache is not present, the
   * queries are directed at the // database instead.
   * 
   * 
